...
./build.py
find . -maxdepth 1 -type f -name "*.deb" | grep -E "^(./)?(libvyosconfig0_|vyos-1x_)" | xargs -I {} cp {} ${ROOTDIR}/packages/
    
# Build telegraf for RPI4
cd ${ROOTDIR}/vyos-build/scripts/package-build/telegraf/
rm -rf telegraf
./build.py
find . -maxdepth 1 -type f -name "*.deb" | grep -E "^(./)?(telegraf_)" | xargs -I {} cp {} ${ROOTDIR}/packages/
